...
RUN yum -y --setopt=tsflags=nodocs install yarn
...
RUN yum --enablerepo=forensics --setopt=tsflags=nodocs install -y libvhdi-tools

# monit to keep an eye on processes
RUN yum  --setopt=tsflags=nodocs -y install monit
ADD monit-services /etc/monit.d/services

# Update OS
RUN yum --setopt=tsflags=nodocs -y update && yum --setopt=tsflags=nodocs -y upgrade && yum clean all
